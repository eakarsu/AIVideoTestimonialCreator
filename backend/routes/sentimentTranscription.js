...
// Transcription with emotion/sentiment markers timeline-aligned
// Feature: sentiment-transcription
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'sentiment-transcription', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Transcription with emotion/sentiment markers timeline-aligned. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'sentiment-transcription', title: 'Transcription with emotion/sentiment markers timeline-aligned', project: 'AIVideoTestimonialCreator' });
});
