...
// Testimonial quality scorer (emotional impact, clarity, length)
// Feature: testimonial-quality-scorer
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'testimonial-quality-scorer', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Testimonial quality scorer (emotional impact, clarity, length). Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'testimonial-quality-scorer', title: 'Testimonial quality scorer (emotional impact, clarity, length)', project: 'AIVideoTestimonialCreator' });
});
