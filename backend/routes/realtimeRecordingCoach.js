...
// Real-time recording coach giving feedback during capture
// Feature: realtime-recording-coach
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'realtime-recording-coach', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Real-time recording coach giving feedback during capture. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'realtime-recording-coach', title: 'Real-time recording coach giving feedback during capture', project: 'AIVideoTestimonialCreator' });
});
