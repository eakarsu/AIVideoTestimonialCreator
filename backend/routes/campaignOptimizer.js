...
// Testimonial campaign optimizer recommending selection and ordering
// Feature: campaign-optimizer
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, feature: 'campaign-optimizer', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Testimonial campaign optimizer recommending selection and ordering. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'campaign-optimizer', title: 'Testimonial campaign optimizer recommending selection and ordering', project: 'AIVideoTestimonialCreator' });
});
