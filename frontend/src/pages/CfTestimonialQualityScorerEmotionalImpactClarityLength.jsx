...
// Feature: Testimonial quality scorer (emotional impact, clarity, length)
...
export default function CfTestimonialQualityScorerEmotionalImpactClarityLength() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-testimonial-quality-scorer-emotional-impact-clarity-length/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-testimonial-quality-scorer-emotional-impact-clarity-length', project: 'AIVideoTestimonialCreator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-testimonial-quality-scorer-emotional-impact-clarity-length/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Testimonial quality scorer (emotional impact, clarity, length)</h1>
