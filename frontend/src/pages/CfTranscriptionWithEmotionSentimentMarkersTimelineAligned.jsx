...
// Feature: Transcription with emotion/sentiment markers timeline-aligned
...
export default function CfTranscriptionWithEmotionSentimentMarkersTimelineAligned() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-transcription-with-emotion-sentiment-markers-timeline-aligned/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-transcription-with-emotion-sentiment-markers-timeline-aligned', project: 'AIVideoTestimonialCreator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-transcription-with-emotion-sentiment-markers-timeline-aligned/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Transcription with emotion/sentiment markers timeline-aligned</h1>
