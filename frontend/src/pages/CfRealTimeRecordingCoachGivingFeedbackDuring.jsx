...
// Feature: Real-time recording coach giving feedback during capture
...
export default function CfRealTimeRecordingCoachGivingFeedbackDuring() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-real-time-recording-coach-giving-feedback-during-capture/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-real-time-recording-coach-giving-feedback-during-capture', project: 'AIVideoTestimonialCreator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-real-time-recording-coach-giving-feedback-during-capture/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Real-time recording coach giving feedback during capture</h1>
