...
// Feature: Testimonial campaign optimizer recommending selection and ordering
...
export default function CfTestimonialCampaignOptimizerRecommendingSelectionAndOrdering() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-testimonial-campaign-optimizer-recommending-selection-and-ordering/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-testimonial-campaign-optimizer-recommending-selection-and-ordering', project: 'AIVideoTestimonialCreator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-testimonial-campaign-optimizer-recommending-selection-and-ordering/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Testimonial campaign optimizer recommending selection and ordering</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVideoTestimonialCreator</p>
      </div>
