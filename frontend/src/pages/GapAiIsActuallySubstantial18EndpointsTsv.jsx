...
export default function GapAiIsActuallySubstantial18EndpointsTsv() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-ai-is-actually-substantial-18-endpoints-tsv-claim/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-ai-is-actually-substantial-18-endpoints-tsv-claim', project: 'AIVideoTestimonialCreator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-ai-is-actually-substantial-18-endpoints-tsv-claim/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">AI is actually substantial (~18 endpoints); TSV claim of 0 is the data error</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIVideoTestimonialCreator</p>
      </div>
